...
## Serena Workflow
- Use Serena MCP as the default code-navigation and refactoring layer for this repo.
- Start repo work by activating `/Users/ilham/Developer/personal/Kubectui` in Serena and checking onboarding.
- Prefer Serena `get_symbols_overview`, `find_symbol`, `find_referencing_symbols`, `find_implementations`, and `search_for_pattern` before broad file reads.
- Prefer Serena symbol edits (`replace_symbol_body`, `insert_before_symbol`, `insert_after_symbol`, `rename_symbol`, `safe_delete_symbol`) when changing whole functions, methods, types, or symbols.
- Use shell tools (`rg`, `fd`, `cargo`, `git`, `gh`) for non-code files, exact text searches, build/test/git workflows, and when Serena cannot answer the question directly.
- Keep exploration token-efficient: read only needed symbols or narrow line ranges; avoid opening full large files like `src/main.rs`, `src/workbench.rs`, or `src/k8s/client.rs` unless symbol tools are insufficient.
- After Serena edits, still run the repo quality gates below; Serena is navigation/editing help, not a replacement for compile/test verification.
