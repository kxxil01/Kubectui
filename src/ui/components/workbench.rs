...
fn render_empty_state(frame: &mut Frame, area: Rect) {
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from("  [,] previous tab, [.] next tab."),
...
fn render_exec_tab(frame: &mut Frame, area: Rect, tab: &crate::workbench::ExecTabState) {
    ...
    let header_lines = vec![
        ...
        Line::from(Span::styled(
            if tab.command_mode {
                "[z] maximize/restore  [,/.] tabs  [Ctrl+W] close  [r] restart  [t] external  [y] copy  [S] save  [i/Enter] input  [Esc] back"
            } else {
...
mod tests {
    ...
    fn exec_command_mode_renders_control_hints() {
        ...
        assert!(rendered.contains("[t] external"));
